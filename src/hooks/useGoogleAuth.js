...
import { loadGoogleScript, initializeGoogleAuth, setGoogleCallback, isGoogleAuthAvailable } from '@/lib/google-auth';
...
export function useGoogleAuth() {
    ...
    const registerWithGoogle = async (additionalData = {}) => {
        ...
        return new Promise((resolve, reject) => {
            setGoogleCallback(async (response) => {
                try {
                    const result = await handleGoogleRegister(response.credential, additionalData);
                    resolve(result);
                } catch (error) {
                    reject(error);
                }
